...
router.get('/', function(req, res) {
    ...
    if(ss.usrLevel == '000' || ss.usrLevel == '001' || ss.usrLevel == '002') {
        ...
        conn.query('select count(*) as cnt from assist_qna_inf_tbl' + addSQL
            ...
            + ' then "답변중" else "" end  as replyyn from assist_qna_inf_tbl, (SELECT @rownum:=' + skip + ') TMP' + addSQL
...
router.get('/view/:no', function(req, res) {
    ...
    conn.query('select @rownum:=@rownum+1 as num, qno, name, usr_no, email, telno, title, content,'
        ...
        + ' DATE_FORMAT(reply_ins_dt, "%Y-%m-%d") as replydate from assist_qna_inf_tbl, (SELECT @rownum:=0) TMP where qno = ?',
        [req.params.no],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
...
router.post('/reply', function(req, res) {
    ...
    console.log("게시글 수정 처리");
    conn.query('update assist_qna_inf_tbl set reply_yn = "Y", reply_id = ?, reply_name = ?, reply_comment = ?, reply_ins_dt= now() where qno = ?;',
