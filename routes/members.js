...
router.get('/', function(req, res) {
    ...
    if(ss.usrLevel == '000' || ss.usrLevel == '001' || ss.usrLevel == '002') {
        ...
        conn.query('select count(*) as cnt from c_inf_tbl ' + addSQL1
            ...
            + ' DATE_FORMAT(insert_dt,"%Y-%m-%d") as date'
            + ' from c_inf_tbl x, (SELECT @rownum:=' + skip + ') TMP' + addSQL2
...
router.get('/view/:no', function(req, res) {
    ...
    conn.query('select c_no as no, c_id as id, AES_DECRYPT(UNHEX(c_pwd),"jtlab") as pwd, c_name as name, c_addr1 as address1, c_addr2 as address2, c_postno as postno, c_email as email, c_sex as sex, c_saup_no as saupno, c_user_tp as usertype, c_auth_level as level, c_birth as birth, c_auth_cd as authcd, c_jumin_no as juminno, c_inf_use_yn as infoyn from c_inf_tbl where c_no = ?',
        [req.params.no],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
...
router.get('/delete/:no', function(req, res) {
    ...
    conn.query('select @rownum:=@rownum+1 as num, c_no as no, c_id as id, AES_DECRYPT(UNHEX(c_pwd),"jtlab") as pwd, c_name as name, c_addr1 as address1, c_addr2 as address2, c_postno as postno, c_email as email, c_sex as sex, c_bus_no as saupno, c_meb_typ as usertype, c_auth_level as level, c_birth as birth, c_auth_cd as authcd, c_jumin_no as juminno, c_inf_use_yn as infoyn from c_inf_tbl, (SELECT @rownum:=0) TMP',
        [],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
