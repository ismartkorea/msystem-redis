...
router.get('/', function(req, res, next) {
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, x.order_no as orderNo, y.service_nm as svcNm,'
        ...
        + ' FROM pay_info_tbl x, order_his_inf_tbl y, (SELECT @rownum:=0) TMP WHERE x.order_no = y.order_no AND x.usr_id = y.usr_id'
        ...
        + ' case when x.refund_yn = "Y" then "있음" when x.refund_yn = "N" then "없음" else "" end as refundYn,'
        + ' x.refund_req_yn as refundReqYn, x.use_term_days as useTermDays, DATE_FORMAT(x.use_end_date,"%Y-%m-%d") as useEndDate'
        + ' FROM pay_info_tbl x, order_inf_tbl y, (SELECT @rownum:=0) TMP WHERE x.order_no = y.order_no AND x.usr_id = y.usr_id'
        + ' AND x.usr_id = ? GROUP BY x.pay_code ORDER BY x.pay_date DESC, x.insert_dt DESC, x.update_dt DESC;'
        + ' SELECT count(req_no) as cnt FROM assist_qna_tbl WHERE ins_usr_id = ? AND view_yn = "N"'
        + ' AND process_status NOT IN ("") ORDER BY upd_dt desc;'
        ...
        function(err, results) {
            if(err) {
                ...
                res.render('./assist/mypage', {
                    'title': '마이 페이지',
                    'rList' : results[0],
                    'rList1' : results[1],
                    'count' : results[2][0].cnt,
                    'tCount' : results[3][0].tCnt,
                    'nList' : results[4],
                    'session' : ss
                });
            }
...
router.get('/paid/view/:payCode', function(req, res) {
    ...
    + ' case when x.refund_yn = "Y" then "있음" when x.refund_yn = "N" then "없음" else "" end as refundYn,'
    + ' x.refund_req_yn as refundReqYn, x.use_term_days as useTermDays, DATE_FORMAT(x.use_end_date,"%Y-%m-%d") as useEndDate'
    + ' FROM pay_info_tbl x, order_inf_tbl y, order_detail_inf_tbl z, (SELECT @rownum:=0) TMP WHERE x.order_no = y.order_no'
    ...
    conn.query(SQL, [payCode, usrId], function(err, results) {
        if(err) {
            ...
            res.status(200).render('./assist/mypagePayDetailPopup', {'rList' : results, 'session' : ss});
        }
    });
    conn.end();
...
router.get('/service/his/view/:orderNo', function(req, res, next) {
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, x.p_code as pCode, x.p_nm as pNm, x.p_div as pDiv,'
        ...
        + ' FROM order_detail_his_inf_tbl x, (SELECT @rownum:=0) TMP WHERE x.order_no = ? ORDER BY x.p_code ASC, sort_no ASC',
        [orderNo],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
                ...
                res.render('./assist/mypageDetailPopup', {title: '마이 서비스 상세 조회', rList : results, session : ss});
            }
        });
    conn.end();
...
router.get('/service/use/view/:orderNo', function(req, res, next) {
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, x.p_code as pCode, x.p_nm as pNm, x.p_div as pDiv, x.p_type as pType,'
        + ' CASE WHEN x.p_div = "M" THEN "기본" WHEN x.p_div = "O" THEN "옵션" ELSE "" END as pDivNm, FORMAT(x.p_price, 0) as pPrice'
        + ' FROM order_detail_inf_tbl x, (SELECT @rownum:=0) TMP WHERE x.order_no = ? ORDER BY x.p_code ASC, sort_no ASC',
        [orderNo],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
    ...
    conn.query('SELECT @rownum:=@rownum+1 as num, x.p_code as pCode, x.p_nm as pNm, x.p_div,'
        ...
        function(err, results) {
            if(err) {
                ...
                console.log('routes mypage Detail View Result !!!');
                res.render('./assist/mypageDetailPopup3', {title: '마이 서비스 상세 조회', rList : results, session : ss});
            }
        });
    conn.end();
...
router.get('/service/refund/view/:orderNo', function(req, res, next) {
    ...
    conn.query('SELECT order_no as orderNo, FORMAT(p_total_price, 0) as totalPayPrice, DATE_FORMAT(pay_date, "%Y-%m-%d") as payDate'
        + ' FROM pay_info_tbl WHERE order_no = ? AND usr_id = ?;'
        ...
        + ' FROM pay_info_tbl x, order_detail_inf_tbl y'
...
//console.log('result : ', JSON.stringify(results));
                res.render('./assist/mypageRefundPopup', {'result' : results[0][0], 'rList' : results[1], 'session' : ss});
            }
        }
    );
    conn.end();
...
console.log('>>> memo : ', memo);
    ...
    var conn = mysql.createConnection(config);
    conn.connect();
    conn.query('SELECT a.cnt as cnt FROM (SELECT @rownum:=@rownum+1 as num, COUNT(order_no) as cnt FROM pay_info_tbl,'
        + ' (SELECT @rownum:=0) TMP WHERE pay_result = "wait" ORDER BY order_no DESC) a WHERE num = 1;'
        + 'SELECT COUNT(order_no) as cnt FROM pay_info_tbl WHERE refund_req_yn = "Y" AND order_no = ?;',
        [orderNo],
        function(err, results) {
            if(err) {
                console.log('err : ', err);
...
//console.log('환불/승인 건수 다 있음');
                    ...
                    console.log('환불신청 처리.');
                    conn.query('UPDATE pay_info_tbl SET refund_req_yn = "Y", refund_req_date = now(), update_dt = now(),'
...
router.get('/qna', function(req, res) {
    ...
    var usrId = ss.usrId !=null ? ss.usrId : '';
    var SQL1 = 'SELECT count(req_no) as cnt FROM assist_qna_tbl WHERE ins_usr_id = ? and view_yn = "N" and process_status NOT IN ("") order by upd_dt desc;';
    var SQL2 = ' SELECT count(req_no) as tCnt FROM assist_qna_his_tbl WHERE process_status NOT IN ("") and ins_usr_id = ?;';
    var SQL3 = ' SELECT req_no as reqNo, case when process_status = "tmp" then "임시저장" when process_status = "prc" then "진행중"'
        + ' when process_status = "sav" then "임시저장" when process_status = "mts" then "답변작성중" when process_status = "rct" then "진행대기"'
        + ' when process_status = "cmp" then "완료" when process_status = "nop" then "장기미처리" else "" end as processStatus,'
        + ' DATE_FORMAT(ins_dt,"%Y년%m월%d일 %H시%i분") as insDt, ins_usr_nm as insUsrNm,'
        + ' DATE_FORMAT(upd_dt,"%Y-%m-%d") as updDt, upd_usr_nm as updUsrNm FROM assist_qna_tbl'
        + ' WHERE view_yn = "N" AND process_status NOT IN ("") AND ins_usr_id = ? ORDER BY upd_dt DESC LIMIT 5;';
    var SQL4 = ' SELECT @rownum:=@rownum+1 as num, x.dno as dNo, x.data_nm as dataNm, x.model_no as modelNo,'
    ...
    var SQL5 = 'SELECT @rownum:=@rownum+1 as num, qno as qNo, name as name, usr_no as usrNo, email as email, telno as telNo,'
        ...
        + ' FROM assist_qna_inf_tbl, (SELECT @rownum:=0) TMP WHERE usr_no = ?;';
...
router.get('/qna/view/:qNo', function(req, res, next) {
    ...
    conn.query('select qno as qNo, name as name, usr_no as usrNo, email as email, telno as telNo,'
        ...
        + ' from assist_qna_inf_tbl where qNo = ? and usr_no = ?',
        [qNo, usrNo],
        function(err, results) {
            if(err) {
                console.log('error : ', err.message);
                ...
                res.render('./assist/myqnaDetailPopup', {title: '1:1문의 상세', result : results[0], session : ss});
            }
        }
    );
    conn.end();
