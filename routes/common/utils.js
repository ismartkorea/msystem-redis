...
Util.prototype.fncRemainDay = function(endDt) {
    ...
    let nowMonth = (dt.getMonth()+1)>9 ? ''+(dt.getMonth()+1) : '0'+(dt.getMonth()+1);
    let nowDay = dt.getDate()>9 ? ''+dt.getDate() : '0'+ dt.getDate();
    let nowDate = nowYear.toString() + "" + nowMonth.toString() + "" + nowDay.toString();

    //var startDate = new Date(startDt.substr(0,4), startDt.substr(4,2)-1, startDt.substr(6,2));
    let endDate = new Date(endDt.substr(0,4), endDt.substr(4,2)-1, endDt.substr(6,2));
    let nDate = new Date(nowDate.substr(0,4), nowDate.substr(4,2)-1, nowDate.substr(6,2));

    let interval = endDate - nDate;

    //var remainYear = parseInt(interval / year);
    //var remainMonth = parseInt(interval / month);
    let remainDay = parseInt(interval / day);
